...
var makeLogger = function (modifier) {
  ...
  function log(msg) {
    if ('stream' in msg) {
      matrix.write(modifier(msg.stream));
    } else if ('status' in msg) {
      if ('id' in msg) {
        matrix.addLine(msg.id);
        if ('progress' in msg) {
          matrix.writeToLine(msg.id, modifier(msg.id + ': ' + msg.status + ' ' + msg.progress));
        } else {
          matrix.writeToLine(msg.id, modifier(msg.id + ': ' + msg.status));
        }
      } else {
        matrix.write(modifier(msg.status));
      }
    }
  }
